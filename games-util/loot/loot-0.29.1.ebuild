...
src_prepare() {
	...
	sed -e "s|lootDocsPath_ = appPath.parent_path() / \"share\" / \"doc\" / \"loot\";|lootDocsPath_ = appPath.parent_path() / \"share\" / \"doc\" / \"${PF}\" / \"html\" ;|" \
		-i src/gui/state/loot_paths.cpp || die
	# Change tests to work with this value
	sed -e "s|EXPECT_EQ(std::filesystem::u8path(\"prefix\") / \"share\" / \"doc\" / \"loot\",|EXPECT_EQ(std::filesystem::u8path(\"prefix\") / \"share\" / \"doc\" / \"${PF}\" / \"html\",|" \
		-i src/tests/gui/state/loot_paths_test.h || die

	# Disable update checking by default. Otherwise you have to manually give it a commit.
	sed -e 's/bool enableLootUpdateCheck_{true};/bool enableLootUpdateCheck_{false};/' \
		-i src/gui/state/loot_settings.h || die
	# Change tests to reflect this changed default
	sed -e 's/EXPECT_TRUE(settings_.isLootUpdateCheckEnabled());/EXPECT_FALSE(settings_.isLootUpdateCheckEnabled());/' \
		-i src/tests/gui/state/loot_settings_test.h || die
}
