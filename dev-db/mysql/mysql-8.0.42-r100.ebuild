...
IUSE="cjk cracklib debug jemalloc latin1 numa +perl profiling router selinux +server systemd tcmalloc test test-install"
RESTRICT="!test? ( test )"
...
src_configure() {
	...
	local mycmakeargs=(
		...
		# Mysql assumes its binaries and libraries are in $PREFIX/dir. So that rpath of form $ORIGIN..dir works.
		-DCMAKE_INSTALL_PREFIX="${EPREFIX}/usr/$(get_libdir)/mysql-${SLOT}"
		...
		-DINSTALL_DOCDIR="${EPREFIX}/usr/share/doc/${PF}"
		-DINSTALL_DOCREADMEDIR="${EPREFIX}/usr/share/doc/${PF}"
		-DROUTER_INSTALL_DOCDIR="${EPREFIX}/usr/share/doc/${PF}"
		-DINSTALL_MANDIR="${EPREFIX}/usr/share/mysql-${SLOT}/man"

		-DINSTALL_MYSQLSHAREDIR="${EPREFIX}/usr/share/mysql-${SLOT}"
		-DINSTALL_SUPPORTFILESDIR="${EPREFIX}/usr/share/mysql-${SLOT}"

		-DINSTALL_INCLUDEDIR="${EPREFIX}/usr/include/mysql-${SLOT}"
	...
	if use server ; then
		...
		use systemd && mycmakeargs+=( -DSYSTEMD_SERVICE_NAME="mysqld-${SLOT}" )
...
src_test() {
	...
	sed \
		...
		"${FILESDIR}"/my.cnf-8.0.distro-client-r100 \
		"${FILESDIR}"/my.cnf-8.0.distro-server-r100 \
			> "${T}"/my.cnf || die
...
src_install() {
	...
	sed -e "s|@SLOT@|${SLOT}|" "${FILESDIR}/my.cnf-8.0.distro-client-r100" > "${TMPDIR}/50-distro-client.cnf" || die
	eprefixify "${TMPDIR}/50-distro-client.cnf"
	doins "${TMPDIR}/50-distro-client.cnf"

	mycnf_src="my.cnf-8.0.distro-server-r100"
...
pkg_config() {
	...
	local mysql_path="${EROOT}/usr/$(get_libdir)/mysql-${SLOT}"

	local mysqld_binary="${mysql_path}/bin/mysqld"
	if [[ ! -x "${mysqld_binary}" ]] ; then
		die "'${mysqld_binary}' not found! Please re-install ${CATEGORY}/${PN}!"
	fi

	local mysql_binary="${mysql_path}/bin/mysql"
	if [[ ! -x "${mysql_binary}" ]] ; then
		die "'${mysql_binary}' not found! Please re-install ${CATEGORY}/${PN}!"
	fi

	local my_print_defaults_binary="${mysql_path}/bin/my_print_defaults"
	...
	"${mysqld_binary}" --verbose --help >"${helpfile}" 2>/dev/null
	...
	"${mysql_path}/bin/mysql_tzinfo_to_sql" "${EROOT}/usr/share/zoneinfo" >> "${tz_sql}" 2>/dev/null
	if [[ $? -ne 0 ]] ; then
