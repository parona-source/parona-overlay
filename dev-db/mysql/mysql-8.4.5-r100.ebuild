...
IUSE="cjk cracklib debug jemalloc numa +perl profiling router selinux +server systemd tcmalloc test test-install"
RESTRICT="!test? ( test )"
...
src_test() {
	...
	sed \
		...
		-e "s|@SLOT@|${SLOT}|" \
		"${FILESDIR}"/my.cnf-8.4.distro-server-r100 > "${T}"/my.cnf || die
...
src_install() {
	...
	sed -e "s|@SLOT@|${SLOT}|" "${FILESDIR}/my.cnf-8.4.distro-client-r100" > "${TMPDIR}/50-distro-client.cnf" || die
	...
	if use server; then
		...
		mycnf_src="my.cnf-8.4.distro-server-r100"
