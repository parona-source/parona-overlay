...
EPYTEST_PLUGINS=()
# FAILED tests/test_update.py::TestUpdate::test_update - FileExistsError: [Errno 17] File exists: '/var/tmp/portage/app-admin/dotdrop-1.17.0/homedir/.config'
#EPYTEST_XDIST=1
...
src_prepare() {
	...
	sed -e '/from halo/d' -i tests-ng/tests_launcher.py || die

	# directories getting removed on failure doesnt help with debugging.
	sed -e '/clear_on_exit/d' -i tests-ng/*.sh || die

	# excise coverage
	sed -e 's/hash coverage/false/' -i tests-ng/*.sh || die
...
python_test() {
	...
	${EPYTHON} tests-ng/tests_launcher.py -n -p "$(get_makeopts_jobs)" || die
}
